...
export default function SubmitWork() {
  ...
  useEffect(() => {
    const fetchGig = async () => {
      ...
      if (
        !snap.exists() ||
        snap.data().status !== "IN_PROGRESS" ||
        snap.data().assignedFreelancerId !== user.uid
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateDoc(doc(db, "gigs", gigId), {
        status: "SUBMITTED",
        submittedFileLink: form.fileLink,
      ...
      <div className="flex">
        ...
        <main className="ml-56 flex-1 p-8 max-w-2xl">
          ...
          <div className="bg-[#161614] border border-white/6 rounded-xl px-6 py-5 mb-6">
            <div className="text-xs text-[#5F5E5A] mb-1">Submitting work for</div>
          ...
          <div className="bg-[#161614] border border-white/6 rounded-xl p-7">
            <h2 className="text-base font-bold mb-6" style={{ fontFamily: "'Syne', sans-serif" }}>
              Submit your work
            </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-[#888780]">
                  File / link <span className="text-[#5F5E5A]">(GitHub, Google Drive, etc.)</span>
                </label>
                <input
                  name="fileLink"
                  value={form.fileLink}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-[#888780]">Completion message</label>
                <textarea
              ...
              <div className="bg-[#EF9F27]/6 border border-[#EF9F27]/15 rounded-lg px-4 py-3 text-xs text-[#FAC775] leading-relaxed">
                Once submitted, the client will review your work and either accept it or request changes.
              </div>
